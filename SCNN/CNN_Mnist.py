...
def train_model(model, train_data, train_label, valid_data, valid_label, num_epochs=20, save=True):
    """训练模型"""
    batch_size = 64
...
if __name__ == '__main__':
    ...
    train_size = 10000  # 若要全部训练需要设置为50k
    valid_size = 1000
    # 创建模型
    model = LeNet5()
    start = time.time()
    model = train_model(model, x_train[:train_size], y_train[:train_size],
                        x_valid[:valid_size], y_valid[:valid_size], num_epochs=20, save=False)
