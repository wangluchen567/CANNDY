...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        # 打印相关信息
        print("Epoch [{:d}/{:d}] | Loss {:.4f} | Train Acc {:.4f} | Valid Acc {:.4f} | Test Acc {:.4f} | Time(s) {:.4f}"
              .format(epoch + 1, num_epochs, ces_loss, train_acc, valid_acc, test_acc, time_list[-1]))

    print()
    # 计算整个数据集的准确率
    acc = evaluate(model, features, labels, np.ones(features.shape[0], dtype=bool))
    print("Full data accuracy {:.2%}".format(acc))
    plot_state(train_loss, accuracies)
